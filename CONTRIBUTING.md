...
We're working hard to make *biocore* the most powerful JavaScript library for working with biocoin. Our goal is to have *biocore* be a library that can be used by anyone interested in biocore, and to level expertise differences with great design and documentation.
...
These are some global design goals in biocore that any change must adhere.
...
In particular, you may notice that some names in this library are quite long for the average JavaScript user. That's because we prefer a long but comprehensible name than an abbrebiotion that might confuse new users.
...
When possible, biocore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of biocore methods. For example, calling `JSON.stringify` on an biocore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
There's a module called `util/preconditions`, loosely based on `preconditions.js`, based on `guava`, that we use for state and argument checking. It should be tribiol to use. We recommend using it on all methods, in order to improve robustness and consistency.
...
#### T3 - Require 'biocore' and Look up Classes from There
...
```javascript
var biocore = require('../');
var PublicKey = biocore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add biocore git@github.com:biocore/biocore.git
git pull --rebase biocore master
```
...
```
Finally go to [github.com/biocoin/biocore](https://github.com/biocore/biocore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of biocore.

If you have any questions feel free to post them to
[github.com/biocoin/biocore/issues](https://github.com/biocoin/biocore/issues).
